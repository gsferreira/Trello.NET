	...
	internal static class Guard
	{
		...
		public static void InRange(int parameter, int min, int max, string parameterName)
		{
			if (parameter < min || parameter > max)
				throw new ArgumentOutOfRangeException(string.Format("Parameter {0} is out of range (must be between {1} and {2})", parameterName, min, max));
		}
		...
		public static void LengthBetween(string parameter, int min, int max, string parameterName)
		{
			...
			if (parameter.Length < min || parameter.Length > max)
				throw new ArgumentOutOfRangeException(string.Format("Length of string parameter {0} is out of range (must be between {1} and {2})", parameterName, min, max));
		}
		...
		{
			if (parameter <= 0)
				throw new ArgumentOutOfRangeException(String.Format("Parameter {0} must be a positive number",parameterName));
		}
	}
}